...
from .models import StudentDB, Edit_Details,Notifications, Appliedproject
...
from professor.models import Register,project_desc
...
def profile(request,username):
    if request.session.has_key('username'):
        ...
        #form = list(tmp.values())  #show any new added project profile only i.e. if (the project id in tmp is not in notification database) and (if in notification database than boolean field is false),then append it to new list and save project_desc in n_text field of notification and set boolean variable in notification to true
        c=tmpt.count()
        ids=[]
        for i in range(0,c):
            ids.append(tmpt.values()[i]['projectid_id'])
        print(ids)
        #ids=list(tmpt.values('projectid_id'))
        form=project_desc.objects.exclude(pk__in=ids)
        #form.extend(tmp)
        print(form)
        l=list(form.values())
        for i in range(len(l)):
            id=l[i]['id']
            idd=project_desc.objects.get(pk=id)
            f,t = Notifications.objects.get_or_create(stdid=s,projectid=idd,n_text=str(user))
            if t:
                f.save()
...
def notify(request,username):
    if request.session.has_key('username'):
        ...
        tmp = project_desc.objects.filter(c_branch=sbranch , c_course=scourse)
        #tmp= project_desc.objects.all()
...
def listproject(request):
    if request.session.has_key('username'):
        ...
        print(sbranch)
        print(scourse)
        tmp= project_desc.objects.filter(c_branch=sbranch,c_course=scourse) #need to add check that student is qualified
        form=list(tmp.values())
        print(form)
        return render(request, 'student/list_project.html', {'form':form,'username':user})
    else:
...
def get_offer(request):
    if request.session.has_key('username'):
        ...
        form=Appliedproject.objects.filter(stdid= sid)# retrieve offer letters from applied project with projectid and attachment download link
...
def download(request):
    path_to_file='D:\\Github\\RI_Portal\\media\\documents\\1.txt'
...
def resumed(request,username):
    path_to_file='D:\\Github\\RI_Portal\\media\\documents\\'+username+'.pdf'
    f = open(path_to_file, 'rb')
    print(f.readlines())
    myfile = File(f)
